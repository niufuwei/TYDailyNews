...
//        UrlImageButton *imageView = [[UrlImageButton alloc]initWithFrame:CGRectMake(0, 0, 320, imageH)];
        UIImageView *imageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, 320, imageH)];
        //给定网络图片路径
//     [imageView setImageFromUrl:YES withUrl:imageName];
        
        NSRange rang = [imageName rangeOfString:@"http:"];
...
//        [imageView addTarget:self action:@selector(click) forControlEvents:UIControlEventTouchUpInside];
        ...
        UIView *titleBack = [[UIView alloc]initWithFrame:CGRectMake(0, imageView.frame.size.height-30, 320, 30)];
        titleBack.backgroundColor =[UIColor whiteColor];
        titleBack.alpha = 0.5;
        [imageView addSubview:titleBack];
        
        //设置标题文字
        CGRect titleRect = titleFrame;
        titleRect.origin.x += 10;
        UILabel *titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(10, imageView.frame.size.height-30, 300, 30)];
